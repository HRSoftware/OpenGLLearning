...
auto TextureLoader::loadMultipleTexturesFromMaterial(aiMaterial* mat, std::string directory) -> std::vector<Texture>
{
    ...
    for (std::string _typeString : types) {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(_type); i++) {
            ...
            textures.push_back( loadSingleTextureFromMaterial(str, _type, directory));
        }
    }
    return textures;
}

inline auto TextureLoader::loadSingleTextureFromMaterial(aiString str, aiTextureType type, std::string directory) -> Texture
{
...
inline int TextureLoader::createTextureIDFromFile(const char* path, const std::string& directory, bool gamma)
{
